  ...
  const handlePrintPDF = () => {
    ...
    const html = `
      ...
      <body>
        ...
        <p style="font-size: 11pt; line-height: 1.6;">${auditResult.finalOpinion?.reasoning}</p>
...
-----------------------
${auditResult.finalOpinion?.reasoning}
      ...
      <div className="flex-1 flex w-full min-h-0 overflow-hidden">
        ...
        <div className={`w-80 flex flex-col border-r shrink-0 overflow-y-auto custom-scrollbar p-5 space-y-5 ${isDark ? 'border-slate-800 bg-[#0c1224]' : 'border-slate-200 bg-white'}`}>
          ...
          <div className="space-y-2">
            ...
            <div className={`border-2 border-dashed rounded-2xl p-5 text-center transition-all relative ${isDark ? 'border-zinc-800 hover:border-indigo-500/50 bg-[#131c31]/30' : 'border-slate-200 hover:border-indigo-500 bg-slate-50/50'}`}>
              <input 
                type="file" 
                multiple
                    ...
                    <div className="space-y-6">
                      <div className="flex items-start gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-xs font-semibold text-slate-500 mt-1 leading-relaxed">{auditResult.finalOpinion?.reasoning}</p>
                        </div>
                      </div>
                      ...
                      <div className="flex-1 overflow-y-auto pr-1 space-y-3 custom-scrollbar mb-4">
                        ...
                        {chatHistory.map(msg => (
                          <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                            <div className={`max-w-[80%] rounded-2xl px-4 py-2.5 text-xs font-semibold leading-relaxed ${msg.role === 'user' ? 'bg-indigo-650 text-white' : 'bg-slate-100 dark:bg-black/25 text-slate-700 dark:text-slate-200 border border-slate-200/30'}`}>
                              {msg.content}
                            </div>
                          </div>
                        ))}
